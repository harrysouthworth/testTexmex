## Check that ggplot.lmr output matches manually created plots
...
## Check that ggplot.gpdThresh output matches plot.gpdThresh output
m <- lmr(log(ALT.M) ~ log(ALT.B) + as.numeric(dose), data=liver)
liver$r <- resid(m)
...
## Check that ggplot.evmOpt output matches plot.evmOpt output
m <- lmr(log(ALT.M) ~ log(ALT.B) + as.numeric(dose), data=liver)
liver$r <- resid(m)
...
## Check that ggplot.evmSim output matches plot.evmSim output
m <- lmr(log(ALT.M) ~ log(ALT.B) + as.numeric(dose), data=liver)
liver$r <- resid(m)
