test_gumbel <- function(){
  ...
  expect_equal(coef(fit1), fit3$mle, tol=0.001, info="gumbel: matches survival package estimation")
  expect_equal(coef(fit2), fit3$mle, tol=0.001, info="gumbel: matches survival package estimation")

  # check numerical cov matrix  agrees with observed info
  expect_equal(fit1$cov, fit2$cov, tol=.0001, info="gumbel: covariance matrix estimation")
  ...
  expect_equal(coef(fit1),fit3$mle,tol=0.01,info="gumbel: matches ismev package estimation with covariates")
  expect_equal(coef(fit2),fit3$mle,tol=0.01,info="gumbel: matches ismev package estimation with covariates")
}
